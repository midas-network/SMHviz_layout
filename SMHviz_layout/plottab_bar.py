...
def multi_pathogen_notes(pathogen, other_pathogen, website, style=None, ensemble=True):
    ...
    names and additional pathogen associated website are changeable in the output notes.

    :parameter pathogen: Name of the pathogen
    :type pathogen: str
    :parameter other_pathogen: Name of the additional pathogen (string or list of multiple
      pathogens in the same order as the `website` parameter)
    :type other_pathogen: str | list
    :parameter website: Website link to the additional pathogen SMH hub website (string or list of
      multiple website in the same order as the `other_pathogen` parameter)
...
def multi_pathogen_bar(pathogen, other_pathogen, quant_opt=None, sel_quant=0.5, bar_style=None,
                       note_style=None, clearable=False, css_class="plot_bar_sel",
                       css_multi_radio="multi_bar_radio"):
    ...
    - a radio items options of the additional pathogen selected round scenario
    (id: `other-scenario`)
    - a notes with link to additional information

    :parameter pathogen: Name of the principal pathogen
    :type pathogen: str
    :parameter other_pathogen: Dictionary containing the information for the other pathogens for
      the multi-pathogen plots. The dictionary should contain the keys: `scenario` (dictionary
      with `id` and `name` keys), `default_sel` (id(s) of the scenario selected by default),
      `name` (name of the pathogen), `round_int` (integer representing the specific round of the
      associated pathogen), `website` (associated website)
    :type other_pathogen: dict | list
    :parameter quant_opt: Value of the associated quantiles dropdowns. If `None`: [0.05, 0.25, 0.5,
       0.75, 0.95]
    :type quant_opt: list
    ...
    bar = html.Div([
        make_dropdown(pathogen + " Quantile", pathogen.lower() + "-quantile_dropdown", quant_opt,
...
def scen_comp_bar(max_horizon, panel_name, multi_panel=False, sidebar_option=False,
    ...
    Create Scenario Comparison top bar filter containing (depending on the input):
     - "multi_panel"
         - if the parameter is `True`: include a RadioItems with each items being a potential
           panels, value for the panel names should be included in the parameter `panel_name`
           (first value selected by default)
         - if the parameter is `False`, the RadioItems is hidden (the panel names are still
           required, first value selected by default)
     - "sidebar_option":
         - if the parameter is `True`: include a slider for each possible weeks, value for the
           weeks should be included with the parameter `max_horizon`
    ...
    :parameter multi_panel: Boolean to show or hide a RadioItems component with each panel as
        options
...
def spaghetti_bar(min_slide=10, max_slide=100, step_slide=10, checkbox_median=True,
    ...
    - a slider with the number of individual trajectories to plot (10, by default)
        (id: "sample-slider")
    - a notes associated with the slider and plot performance impact
    - (if checkbox_median is True) a checkbox called "Show Median" (returns `True` if selected)
        (id: "median-checkbox")
    ...
    :parameter css_med: string, name of the associated CSS element for the median checkbox, see
        documentation
    :type css_med: str
...
def heatmap_bar(model_sel, scen_choice, hide_ens, quant_opt=None, sel_quant=0.5, method_list=None,
    ...
    Create Heatmap (or Spatiotemporal Waves) top bar filter containing:
        ...
        - Second line:
            - Dropdown (clearable) with the scenario associated with the round (second one, by
                default)
            - Dropdown with the quantile to plot

    :parameter model_sel: a Div component with the team-model dropdown information
    :type model_sel: Div
    :parameter scen_choice: list of scenario id associated with the round (need to have at least
        two scenarios)
    :type scen_choice: list
    :parameter hide_ens: Boolean to hide (True) or show (False) the "Ensemble Checkbox" (show
        available additional ensemble in the plot)
    :type hide_ens: bool
    :parameter quant_opt: Value of the associated quantiles dropdowns. If `None`: [0.05, 0.25,
        0.5, 0.75, 0.95]
    :type quant_opt: list
    :parameter sel_quant: Default selected value of the quantiles dropdowns
    :type sel_quant: int
    ...
    order_radio = make_radio_items("Location Order", id_name="order_heatmap", value="Geographical",
                                   options=["Alphabetical", "Geographical"], inline=True,
                                   css_class=css_h_radio)
    method_dropdown = make_dropdown("Standardization Approach", "method_dropdown",
                                    options=method_list,
...
def sample_peak_bar(tf_options=None, clearable=False, css_class="plot_bar_sel",
                    css_bar_plot="plot_bar"):
    """Create Peak specific top bar filter

    Create Peak top bar filter containing:
        - a Dropdown with team-model information (without Ensemble(s)), with an "Ensemble" (peak
          specific ensemble)
    ...
    :parameter tf_options: List of possible time frames for the Peak plot. First value selected by
      default
      If `None`: ["summer 2023", "winter 2023", "year 2023", "summer 2024", "winter 2024",
      "year 2024"].
    ...
    if tf_options is None:
        tf_options = ["summer 2023", "winter 2023", "year 2023", "summer 2024", "winter 2024",
                      "year 2024"]
    model_tf_sel = make_dropdown(title="Model", id_name='model_dropdown', options="Ensemble",
                                 value="ensemble", css_class=css_class, clearable=clearable)
    tf_drop = make_dropdown(title="Time Frame", id_name='tf_dropdown', options=tf_options,
                            value=tf_options[0], css_class=css_class, clearable=clearable)
    checkbox = make_checkbox("", "ensemble-checkbox",
                             [{"label": "Show Additional Ensemble", "value": "False"}],
                             hide=True, style={})
    plot_bar = html.Div([
...
def make_plot_bar(val_default, max_horizon, hide_ens, sc_panel_name, sc_multi_panel,
    ...
    :parameter sc_multi_panel: Boolean to show or hide a RadioItems component with each panel as
        options, for the scenario comparison plot
    :type sc_multi_panel: bool
    :parameter sc_sidebar_option:  Boolean to show or hide a slider with week information, in the
    ...
    :parameter other_pathogen: List of dictionaries containing the information for the other
        ...
        (dictionary with `id` and `name` keys), `default_sel`(id(s) of the scenario selected by
        default), `name` (name of the pathogen), `round_int` (integer representing the specific
        round of the associated pathogen), `website` (associated website). For the multi-pathogen
        plot (not combined) only the first element of the list will be used.
    :type other_pathogen: list | None
    :parameter plot_tab: The id name of the plot selected tab ("scenario" for example, associated
        with "Scenario Plot")
    :type plot_tab: str
    :parameter quant_opt: Value of the associated quantiles dropdowns. If `None`: [0.05, 0.25, 0.5,
        0.75, 0.95]
    ...
    :type method_list: list
    :parameter tf_options: List of possible time frames for the Peak plot. First value selected by
        default. If `None`: ["summer 2023", "winter 2023", "year 2023", "summer 2024",
        "winter 2024", "year 2024"].
    :type tf_options: list
    :parameter traj_min: Minimum value in the slider, by default 10, for the individual
        trajectories plot only
    :type traj_min: int
    :param traj_max: Maximum value in the slider, by default 100, for the individual trajectories
        plot only
    :type traj_max: int
    :parameter traj_step: Step in the slider, by default 10, for the individual trajectories plot
        only
    :type traj_step: int
    :parameter check_med: Boolean, if True include the "median checkbox" in the output, for the
        individual trajectories plot only
    :type check_med: bool
    ...
    model_sel = make_dropdown(title="Model", id_name='model_dropdown', options=[val_default],
    ...
    radio_yaxis = make_radio_items(
        ...
        value='linear', css_class=css_sel, inline=inline_radio)
    ...
    if plot_tab in ["scenario"]:
        ...
        plot_bar = scen_comp_bar(max_horizon, sc_panel_name, multi_panel=sc_multi_panel,
                                 sidebar_option=sc_sidebar_option, css_class=css_sel,
                                 tooltip=tooltip, radio_comp_style=radio_comp_style)
        ...
        plot_bar = multi_pathogen_bar(pathogen, other_pathogen[0],
                                      quant_opt=quant_opt, sel_quant=sel_quant,
                                      bar_style=multi_bar_style, note_style=multi_note_style,
                                      clearable=clearable, css_class=css_sel,
        ...
        plot_bar = spaghetti_bar(
            min_slide=traj_min, max_slide=traj_max, step_slide=traj_step,
            checkbox_median=check_med, css_med=css_sel,
        ...
        plot_bar = heatmap_bar(model_sel, scen_choice, hide_ens, quant_opt=quant_opt,
                               ...
                               css_class=css_sel,  css_h_radio=css_h_radio, css_h_drop=css_h_drop,
                               css_h_plot=css_bar_plot, style=heatmap_style)
        ...
        order_radio = make_radio_items("Location Order", id_name="order_heatmap",
                                       value="Geographical",
                                       options=["Alphabetical", "Geographical"], inline=True,
                                       css_class=css_h_radio)
